...
def build_dataset_individuals(scores_df, min_cov=0.3):
    """
    Construit un dataset où CHAQUE fichier (Healthy ou Patho) = 1 individu.
    - Label Healthy = 0
    - Label Patho   = score du patient (tel que dans 0list.xlsx)
    Retourne: X (DataFrame), y (Series), sids (list[str]), pids (list[int])
      * sids: identifiants échantillon, ex "0007_H" / "0007_P"
      * pids: id patient num. (utile pour Group CV sans fuite)
    """
    rows, labels, sids, pids = [], [], [], []

    for pid, score in scores_df[["patient", "score"]].itertuples(index=False):
        # Healthy -> label 0
        df_h = load_measure(pid, "Healthy")
        fs_h = extract_features_single(df_h, min_cov=min_cov)
        if fs_h is not None and not fs_h.empty:
        ...
        if fs_p is not None and not fs_p.empty:
            y_val = pd.to_numeric(score, errors="coerce")
            if pd.isna(y_val):
                # si score manquant, on zappe cet échantillon Patho
                pass
            else:
